...
ChannelManager = new JS.Class({
  ...
  registerActiveSubscriber: function(channelId, client) {
    console.log("registerActiveSubscriber(" + channelId + ", client{" + client.sessionId + "})");
  ...
  unregisterSubscriber: function(channelId, client) {
    if (this.channelSubscribers[channelId]) {
      _(this.channelSubscribers[channelId]).remove(client);
    }
  },
  deliverQueuedMessages: function(channelId, client) {
    ...
    console.log("this.channelMessages[channelId].length = " + this.channelMessages[channelId].length);
    while (this.channelMessages[channelId].length > 0) {
      var message = this.channelMessages[channelId].pop();
